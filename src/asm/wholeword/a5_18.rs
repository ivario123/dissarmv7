...
impl ToThumb for A5_18 {
    fn encoding_specific_operations(self) -> thumb::Thumb {
        match self {
            Self::LdrImmediateT3(el) => thumb::LdrImmediate::builder().set_w(Some(false)).set_add(true).set_rt(el.rt).set_rn(el.rn).set_imm(el.imm12.into()).set_index(true).complete().into(),
            Self::LdrImmediateT4(el) => thumb::LdrImmediate::builder().set_w(Some(el.w)).set_add(el.u).set_index(el.p).set_rt(el.rt).set_rn(el.rn).set_imm(el.imm8 as u32).complete().into(),
            Self::Ldrt(el) => thumb::Ldrt::builder().set_rt(el.rt).set_rn(el.rn).set_imm(Some(el.imm8 as u32)).complete().into(),
            Self::LdrRegister(el) => {
                let shift = ImmShift::try_from((Shift::Lsl, el.imm2.into())).unwrap();

                thumb::LdrRegister::builder().set_w(None).set_rt(el.rt).set_rn(el.rn).set_rm(el.rm).set_shift(Some(shift)).complete().into()
            }
            Self::LdrLiteral(el) => thumb::LdrLiteral::builder().set_rt(el.rt).set_add(el.u).set_imm(el.imm12.into()).complete().into(),
        }
    }
}
